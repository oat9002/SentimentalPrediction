...
for wd in words:
    temp = []
    temp.append(wd[0])
    if rulebase.check_emo_in_word(joy_list, wd[0]):
        temp.append('joy')
    else:
        temp.append('')
    if rulebase.check_emo_in_word(sadness_list, wd[0]):
        temp.append('sadness')
    else:
        temp.append('')
    if rulebase.check_emo_in_word(fear_list, wd[0]):
        temp.append('fear')
    else:
        temp.append('')
    if rulebase.check_emo_in_word(angry_list, wd[0]):
        temp.append('angry')
    else:
        temp.append('')
    if rulebase.check_emo_in_word(disgust_list, wd[0]):
        temp.append('disgust')
    else:
        temp.append('')
    if rulebase.check_emo_in_word(surprise_list, wd[0]):
        temp.append('surprise')
    else:
        temp.append('')
    if rulebase.check_emo_in_word(anticipation_list, wd[0]):
        temp.append('anticipation')
    else:
        temp.append('')
    if rulebase.check_emo_in_word(acceptance_list, wd[0]):
